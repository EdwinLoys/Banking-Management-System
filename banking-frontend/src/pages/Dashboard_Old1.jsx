...
const calcEMI = (amount,months,rate) => {
  ...
  const totalInterest = Math.round(amount*rate/100*(months/12)*100)/100;
  const totalPayable  = amount+totalInterest;
  return {monthly:Math.round(totalPayable/months*100)/100, totalInterest, totalPayable};
};
...
const pdfCustomer = c => {
  ...
  .scard{background:#1a3a6b;color:white;border-radius:6px;padding:12px;text-align:center;}
  .sl{font-size:10px;opacity:.8;margin-bottom:4px;}.sv{font-size:16px;font-weight:700;}
  ...
  thead tr{background:#1a3a6b;color:white;}th,td{padding:8px 10px;text-align:left;border-bottom:1px solid #f1f5f9;}
  ...
  <p>Generated: ${new Date().toLocaleString()} | CR Number: ${c.customerNo}</p></div>
  <div class="g4">
    <div class="scard"><div class="sl">CR Number</div><div class="sv">${c.customerNo}</div></div>
    <div class="scard"><div class="sl">Balance</div><div class="sv">LKR ${parseFloat(bal).toLocaleString()}</div></div>
    <div class="scard"><div class="sl">Accounts</div><div class="sv">${c.accounts?.length||0}</div></div>
    <div class="scard"><div class="sl">Loans</div><div class="sv">${c.loans?.length||0}</div></div>
  </div>
  <div class="g2">
  ...
  <h2>Accounts</h2>
  ${c.accounts?.length?`<table><thead><tr><th>Account No</th><th>Type</th><th>Balance</th><th>Created</th></tr></thead><tbody>
  ${c.accounts.map(a=>`<tr><td>${a.accountNumber}</td><td>${a.accountType}</td><td>LKR ${parseFloat(a.balance).toLocaleString()}</td><td>${new Date(a.createdAt).toLocaleDateString()}</td></tr>`).join('')}
  </tbody></table>`:'<p style="color:#94a3b8;font-size:12px">No accounts</p>'}
  <h2>Loans</h2>
  ${c.loans?.length?`<table><thead><tr><th>Loan ID</th><th>Type</th><th>Amount</th><th>Rate</th><th>EMI</th><th>Total Payable</th><th>Status</th><th>%</th></tr></thead><tbody>
  ${c.loans.map(l=>`<tr><td>${l.loanNo}</td><td>${l.loanType||'—'}</td><td>LKR ${parseFloat(l.amount).toLocaleString()}</td><td>${l.interestRate||'—'}%</td><td>LKR ${parseFloat(l.monthlyPayment).toLocaleString()}</td><td>LKR ${parseFloat(l.totalPayable||l.amount).toLocaleString()}</td><td>${l.status}</td><td>${l.progress}%</td></tr>`).join('')}
  </tbody></table>`:'<p style="color:#94a3b8;font-size:12px">No loans</p>'}
  <h2>Transactions</h2>
  ${txs.length?`<table><thead><tr><th>Date</th><th>Type</th><th>Amount</th><th>Description</th></tr></thead><tbody>
  ${txs.slice(0,20).map(t=>`<tr><td>${new Date(t.createdAt).toLocaleDateString()}</td><td>${t.type}</td><td>LKR ${parseFloat(t.amount).toLocaleString()}</td><td>${t.description}</td></tr>`).join('')}
  </tbody></table>`:'<p style="color:#94a3b8;font-size:12px">No transactions</p>'}
...
const pdfBulk = (type,data) => {
  ...
  const rowMap = {
    ...
    Account:    data.map(a=>[a.accountNumber,a.customerNo||'—',a.customerName,a.accountType,`LKR ${parseFloat(a.balance).toLocaleString()}`,new Date(a.createdAt).toLocaleDateString()]),
    Transaction:data.map(t=>['T'+String(t.transactionId).padStart(3,'0'),t.createdAt?.slice(0,10),t.type,t.customerNo||'—',t.customer||'—',t.description,`LKR ${parseFloat(t.amount).toLocaleString()}`]),
    Loan:       data.map(l=>[l.loanNo,l.loanType||'—',l.customerNo,l.customerName,`LKR ${parseFloat(l.amount).toLocaleString()}`,`${l.interestRate||'—'}%`,`LKR ${parseFloat(l.monthlyPayment).toLocaleString()}`,`LKR ${parseFloat(l.totalPayable||l.amount).toLocaleString()}`,`${l.months}mo`,l.status,`${l.progress}%`]),
  };
  const w=window.open('','_blank');
  w.document.write(`<!DOCTYPE html><html><head><title>${type}</title>
  <style>body{font-family:Segoe UI,sans-serif;padding:24px;}.hdr{background:#1a3a6b;color:white;padding:16px 22px;border-radius:6px;margin-bottom:18px;}.hdr h1{font-size:17px;margin:0 0 3px;}.hdr p{margin:0;font-size:11px;opacity:.8;}
  table{width:100%;border-collapse:collapse;font-size:11px;}thead tr{background:#1a3a6b;color:white;}th,td{padding:6px 8px;text-align:left;border-bottom:1px solid #f1f5f9;}tr:nth-child(even) td{background:#f8faff;}
  .foot{margin-top:18px;font-size:10px;color:#94a3b8;text-align:center;border-top:1px solid #f1f5f9;padding-top:8px;}</style></head><body>
  <div class="hdr"><h1>🏛 Bank System — ${type} Report</h1><p>Generated: ${new Date().toLocaleString()} | ${data.length} records</p></div>
  <table><thead><tr>${colMap[type].map(h=>`<th>${h}</th>`).join('')}</tr></thead>
  <tbody>${rowMap[type].map(r=>`<tr>${r.map(c=>`<td>${c}</td>`).join('')}</tr>`).join('')}</tbody></table>
...
export default function Dashboard() {
  ...
  const toast$ = (msg,type='ok') => {
    setToast({msg,type}); setTimeout(()=>setToast({msg:'',type:'ok'}),4500);
  };
  const openModal = (t,d={}) => {
  ...
  const closeModal = () => {
    ...
    setNewCustRes(null); setNewLoanRes(null); setCustSumm(null);
    setLoanStep(1); setSelType(null);
  };
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const safe = p => p.catch(e => { console.warn('API error:', e.config?.url, getErr(e)); return {data:[]}; });
      ...
      else toast$('❌ Cannot reach the server. Make sure the API is running.','err');
  ...
  const findByCR = v =>
    customers.find(c =>
      c.customerNo?.toUpperCase() === v?.trim().toUpperCase() ||
      String(c.customerId) === v?.trim()
    );
  ...
  const loadCustSumm = async cr => {
    ...
    setSummLoad(true);
    try {
      const res = await axios.get(`${API}/api/customer/by-crno/${cr.trim().toUpperCase()}`,{headers});
      setCustSumm(res.data);
  ...
  const doNewCustomer = async () => {
    ...
    try {
      const r = await axios.post(`${API}/api/customer`,
        {fullName:form.name.trim(),email:form.email.trim(),phone:form.phone||''},{headers});
      setNewCustRes(r.data); load();
    } catch(e){ toast$('❌ '+getErr(e),'err'); }
  };
  ...
  const doNewAccount = async () => {
    ...
    try {
      await axios.post(`${API}/api/account/create`,{accountType:form.type,customerId:c.customerId},{headers});
      toast$('✅ Account created'); closeModal(); load();
    } catch(e){ toast$('❌ '+getErr(e),'err'); }
  };
  ...
  const doNewTx = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/api/transaction/${ep}`,
        {accountId:c.accounts[0].accountId,amount:amt,description:form.desc||form.type},{headers});
  ...
  const doNewLoan = async () => {
    ...
    if (!c) { toast$('❌ Customer not found','err'); return; }
    try {
      const r = await axios.post(`${API}/api/loan/apply`,
        {customerId:c.customerId,loanType:selType.type,amount:parseFloat(form.amount),months:parseInt(form.months||selType.minM)},{headers});
  ...
  const doApprove = async id => {
    ...
    try {
      await axios.put(`${API}/api/loan/${id}/approve`,{},{headers});
      toast$('✅ Loan approved and activated'); load();
    } catch(e){ toast$('❌ '+getErr(e),'err'); }
  };
  ...
  const doReject = async id => {
    ...
    if (!reason) return;
    try {
      await axios.put(`${API}/api/loan/${id}/reject`,JSON.stringify(reason),
        {headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json'}});
      toast$('🚫 Loan rejected'); load();
    } catch(e){ toast$('❌ '+getErr(e),'err'); }
  };
  ...
  const doPayEmi = async () => {
    ...
    try {
      const r = await axios.post(`${API}/api/loan/pay-emi`,
        {loanNo,customerNo:crNo,amount:amt,note:form.emiNote||''},{headers});
      setPayResult(r.data);
  ...
  const doDelete = async id => {
    ...
    try {
      await axios.delete(`${API}/api/customer/${id}`,{headers});
  ...
  const doPDF = async id => {
    try {
      ...
      pdfCustomer(r.data);
    } catch { toast$('❌ Could not load customer','err'); }
  };
  ...
  const exportCSV = async type => {
    try {
      ...
      if (type==='Transaction') {
        if(csvFrom) p.append('from',csvFrom);
        if(csvTo)   p.append('to',csvTo);
        if(csvTxType) p.append('type',csvTxType);
      }
  ...
  const emiPreview = selType&&form.amount&&form.months
    ? calcEMI(parseFloat(form.amount),parseInt(form.months),selType.rate) : null;
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        .slogo-icon{width:38px;height:38px;background:white;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:18px;}
        .slogo-t{font-size:15px;font-weight:800;color:white;line-height:1.3;}
        .snav{flex:1;padding:8px 0;}
        .nb{width:100%;display:flex;align-items:center;justify-content:space-between;padding:11px 17px;
          background:none;border:none;color:rgba(255,255,255,.7);font-size:13.5px;font-weight:500;
          cursor:pointer;text-align:left;font-family:'Inter',sans-serif;border-left:3px solid transparent;transition:background .14s;}
        ...
        .srole.Employee{background:#e0f2fe;color:#0369a1;}
        ...
        .rp.Admin{background:#fef3c7;color:#b45309;}
        .rp.Employee{background:#e0f2fe;color:#0369a1;}
        ...
        .ddm{position:absolute;top:calc(100% + 4px);left:0;z-index:400;background:white;border:1px solid #dde3ec;border-radius:9px;box-shadow:0 8px 24px rgba(0,0,0,.12);min-width:190px;overflow:hidden;}
        ...
        .tb.pdf{background:#f0fdf4;color:#16a34a;}
        .tb.del{background:#fef2f2;color:#dc2626;}
        .tb.apr{background:#eff6ff;color:#1a56db;}
        .tb.rej{background:#fef2f2;color:#dc2626;}
        ...
        .lt-icon{font-size:27px;margin-bottom:6px;}
      ...
      {loading && (
        <div className="ls2">
          <div className="sp"/>
          <div style={{fontSize:14,color:'#64748b',fontWeight:500}}>Loading Bank System...</div>
        </div>
      )}
      ...
      <div className="root">
        ...
        <aside className="side">
          <div className="slogo">
            ...
            <div className="slogo-t">Bank<br/>System</div>
          </div>
          <nav className="snav">
            {navItems.map(n=>(
          ...
          <div className="cnt">
            ...
            <div className="qg">
              {[
                {key:'cust',cls:'bl',icon:'👤',title:'Customer Registration',label:'Add Customer ▾',
                  items:[{t:'➕ Add New Customer',fn:()=>openModal('customer')},{t:'👥 View All',fn:()=>setActiveNav('Customer')},{t:'🔍 Customer Summary',fn:()=>openModal('summary')}]},
                ...
                <div key={q.key} className="qc">
                  ...
                  <div className="ddw" onClick={e=>e.stopPropagation()}>
                    ...
                    {dd===q.key&&(
                      <div className="ddm">
                        {q.items.map((it,i)=>(
                          <button key={i} className={`ddi ${it.dim?'dim':''}`}
                            onClick={it.fn && !it.dim ? it.fn : undefined}>
                            {it.t}
                          </button>
                        ))}
                      </div>
            ...
            <div className="rg">
              <div className="rb">
                ...
                {recentCusts.length===0
                  ? <div className="nr">No new customers in last 10 days</div>
              ...
              <div className="rb">
                ...
                {recentAccs.length===0
                  ? <div className="nr">No new accounts in last 10 days</div>
              ...
              <div className="rb">
                ...
                {recentTxs.length===0
                  ? <div className="nr">No recent transactions in last 10 days</div>
                  : recentTxs.slice(0,5).map(t=>{
                    const p = t.type==='Deposit'||t.type==='Transfer In';
                    return (
                      <div key={t.transactionId} className="ri">
            ...
            {(activeNav==='Dashboard'||activeNav==='Customer')&&(
              <div className="card">
                ...
                <div className="sb-bar">
                  <input className="si" placeholder="Search by name, phone, or CR number..."
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {pageC.length===0
                      ? <tr><td colSpan={10} className="ec">No customers found. Click "Add Customer" to get started.</td></tr>
                      : pageC.map(c=>(
                        <tr key={c.customerId}>
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {allAccounts.length===0
                      ? <tr><td colSpan={6} className="ec">No accounts</td></tr>
                      : allAccounts.map(a=>(
                        <tr key={a.accountId}>
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {transactions.length===0
                      ? <tr><td colSpan={7} className="ec">No transactions</td></tr>
            ...
            {activeNav==='Loan'&&(
              <div className="card">
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {loans.length===0
                      ? <tr><td colSpan={12} className="ec">No loans found. Apply for a loan from the Loan Management menu.</td></tr>
                      : loans.map(l=>{
                        ...
                        return (
                          <tr key={l.loanId}>
                            ...
                            <td>
                              ...
                              {!isAdmin && l.status==='Pending' && (
                                <span style={{fontSize:10,color:'#94a3b8'}}>Awaiting Admin</span>
                              )}
                            </td>
                          </tr>
                        );
            ...
            {activeNav==='Audit Log' && isAdmin && (
              <div className="card">
                ...
                {auditLogs.length===0
                  ? <div className="nr">No audit logs yet</div>
                  : auditLogs.map(a=>(
            ...
            {activeNav==='Fraud Alerts' && isAdmin && (
              <div style={{display:'flex',flexDirection:'column',gap:12,marginBottom:14}}>
                {[
                  {title:'🚨 Large Transactions (≥ LKR 100,000)',items:fraud.large,color:'#b91c1c',render:t=>[<span className="cr">{t.customerNo}</span>,<span style={{fontWeight:500}}>{t.customerName}</span>,<span className={`bdg ${t.type==='Deposit'?'bd':'bw'}`}>{t.type}</span>,<span style={{fontWeight:700,color:'#b91c1c'}}>LKR {fmtN(t.amount)}</span>,<span style={{color:'#64748b',fontSize:11}}>{t.createdAt?.slice(0,10)}</span>]},
                  {title:'🚨 Suspicious Activity (Multiple large withdrawals today)',items:fraud.suspicious,color:'#b91c1c',render:s=>[<span className="cr">{s.customerNo}</span>,<span style={{fontWeight:500}}>{s.customerName}</span>,<span style={{color:'#b91c1c',fontWeight:700}}>{s.txCount} withdrawals today</span>,<span style={{fontWeight:700}}>LKR {fmtN(s.totalAmount)}</span>]},
                  {title:'⚠️ Delinquent Loans (No EMI in 35+ days)',items:fraud.delinquent,color:'#b45309',render:l=>[<span className="cr loan">{l.loanNo}</span>,<span className="cr">{l.customerNo}</span>,<span style={{fontWeight:500}}>{l.customerName}</span>,<span style={{color:'#b45309',fontWeight:700}}>LKR {fmtN(l.monthlyPayment)}/mo</span>,<span style={{fontSize:11,color:'#64748b'}}>Last: {l.lastPayment?new Date(l.lastPayment).toLocaleDateString():'Never'}</span>]},
                ].map((sec,si)=>(
                  <div key={si} className="card" style={{marginBottom:0}}>
                    <div className="ct" style={{background:si<2?'#fef2f2':'#fef3c7'}}>
                      {sec.title}
                      <span className="cbg" style={{background:sec.color}}>{sec.items.length}</span>
                    </div>
                    {sec.items.length===0
                      ? <div className="nr">No alerts</div>
                      : sec.items.map((item,ii)=>(
                        <div key={ii} className="fraud-item">
                          {sec.render(item).map((el,ei)=><span key={ei}>{el}</span>)}
                        </div>
                      ))
                    }
                  </div>
                ...
                <table className="tbl">
                  ...
                  <tbody>
                    {customers.filter(c=>c.status==='Active').length===0
                      ? <tr><td colSpan={5} className="ec">No active customers</td></tr>
            ...
            <div className="bg">
              ...
              <div className="card" style={{marginBottom:0}}>
                ...
                {loans.length>1&&(
                  <div className="lsel">
                    {loans.map(l=>(
                      <button key={l.loanId} className={`lpil ${selectedLoan?.loanId===l.loanId?'s':''}`}
                        onClick={()=>{setSelectedLoan(l);setPayResult(null);}}>
                        {lt_icon(l.loanType)} {l.loanNo}
                      </button>
                    ))}
                  </div>
                )}
                ...
                {!activeLoan
                  ? <div style={{padding:24,textAlign:'center',color:'#94a3b8',fontSize:13}}>No loans yet. Apply for a loan from Loan Management.</div>
                  : <>
                    {[
                    ...
                    <div className="prw">
                      <div className="prt"><div className="prf" style={{width:`${activeLoan.progress}%`,background:lt_color(activeLoan.loanType)}}/></div>
                      <div className="prl">{activeLoan.progress}% repaid</div>
                    </div>

                    {activeLoan.status==='Active'&&(
                      <div className="ps">
                        <div className="pt">💳 Pay EMI — {lt_icon(activeLoan.loanType)} {activeLoan.loanType}</div>
                        <input className="pinp" placeholder={`Loan ID e.g. ${activeLoan.loanNo}`} type="text"
                          value={form.emiLoanNo||''} onChange={e=>setForm(f=>({...f,emiLoanNo:e.target.value.toUpperCase()}))}/>
                        <input className="pinp" placeholder="CR Number e.g. CR10000001" type="text"
                          value={form.emiCrNo||''} onChange={e=>setForm(f=>({...f,emiCrNo:e.target.value.toUpperCase()}))}/>
                        <div className="pr2">
                          <input className="pinp" style={{marginBottom:0}}
                            placeholder={`Amount e.g. LKR ${fmtN(activeLoan.monthlyPayment||0)}`}
                            type="text" inputMode="decimal"
                            value={form.emiAmount||''} onChange={e=>setForm(f=>({...f,emiAmount:e.target.value}))}
                            onKeyDown={e=>e.key==='Enter'&&doPayEmi()}/>
                          <button className="pbtn" onClick={doPayEmi}>Pay Now</button>
                        </div>
                        <input className="pinp" placeholder="Note (optional)" value={form.emiNote||''}
                          onChange={e=>setForm(f=>({...f,emiNote:e.target.value}))}/>
              ...
              <div className="card" style={{marginBottom:0}}>
                ...
                <div className="txm">
                  ...
                  {transactions.length===0
                    ? <div style={{padding:'9px 13px',fontSize:12,color:'#94a3b8'}}>No transactions yet</div>
                    : transactions.slice(0,6).map(t=>{
                      const p = t.type==='Deposit'||t.type==='Transfer In';
                      return (
                        <div key={t.transactionId} className="txr">
                          <div>
                            <span style={{color:'#374151'}}>{t.description||t.type}</span>
                            {t.customerNo&&<span style={{marginLeft:5}}><span className="cr" style={{fontSize:10,padding:'1px 5px'}}>{t.customerNo}</span></span>}
                          </div>
              ...
              <div className="card" style={{marginBottom:0}}>
                ...
                <div className="lss">
                  ...
                  {[
                    ...
                    <div key={i} className="lsr">
                      <span style={{color:'#64748b'}}>{s.l}</span>
                      <span style={{fontWeight:700,color:'#1a3a6b'}}>{s.v}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="ovl" onClick={e=>{if(e.target===e.currentTarget)closeModal();}}>
          <div className="mdl">
            ...
            {modal==='customer'&&<>
              ...
              {!newCustRes?<>
                <div className="note">✅ CR number auto-generated (e.g. CR10000001). No manual entry needed.</div>
                <label className="ml">Full Name *</label>
                <input className="mi" placeholder="John Smith" value={form.name||''} onChange={e=>setForm({...form,name:e.target.value})}/>
                <label className="ml">Email *</label>
                <input className="mi" placeholder="john@email.com" type="email" value={form.email||''} onChange={e=>setForm({...form,email:e.target.value})}/>
                <label className="ml">Phone</label>
                <input className="mi" placeholder="0771234567" value={form.phone||''} onChange={e=>setForm({...form,phone:e.target.value})}/>
                <div className="mf">
                  <button className="mb c" onClick={closeModal}>Cancel</button>
                  <button className="mb p" onClick={doNewCustomer}>Create Customer</button>
                </div>
              </>:<>
                <div className="suc">
            ...
            {modal==='summary'&&<>
              ...
              <div style={{display:'flex',gap:8,marginBottom:12}}>
                <input className="mi" style={{marginBottom:0}} placeholder="e.g. CR10000001"
                  value={form.sumCr||''} onChange={e=>setForm({...form,sumCr:e.target.value.toUpperCase()})}/>
                <button className="mb p" style={{flex:'0 0 auto',padding:'9px 16px'}}
                  onClick={()=>loadCustSumm(form.sumCr)}>
                  {summLoad?'...':'Search'}
                </button>
              </div>
              {custSumm&&<>
                <div className="csbox">
                  ...
                  <div className="csgrid">
                    {[{l:'CR Number',v:custSumm.customerNo},{l:'Name',v:custSumm.fullName},{l:'Phone',v:custSumm.phone||'—'},{l:'Status',v:custSumm.status},{l:'Balance',v:`LKR ${fmtN(custSumm.balance||0)}`},{l:'Accounts',v:custSumm.accounts?.length||0},{l:'Total Loans',v:custSumm.totalLoans||0},{l:'Active Loans',v:custSumm.activeLoans||0}].map((x,i)=>(
                      <div key={i} className="csitem">
                        <div className="csitem-l">{x.l}</div>
                        <div className="csitem-v" style={x.l==='Balance'?{color:'#16a34a'}:x.l==='Active Loans'&&custSumm.activeLoans>0?{color:'#dc2626'}:{}}>{x.v}</div>
                      </div>
                    ))}
                  </div>
                  {custSumm.accounts?.map((a,i)=>(
                    <div key={i} style={{background:'white',borderRadius:6,padding:'8px 10px',marginTop:8,border:'1px solid #e2e8f0',fontSize:12}}>
                      <div style={{display:'flex',justifyContent:'space-between'}}>
                        <span style={{fontWeight:600,color:'#1a3a6b'}}>{a.accountType}</span>
                        <span style={{fontWeight:700,color:'#16a34a'}}>LKR {fmt(a.balance)}</span>
                      </div>
            ...
            {modal==='account'&&<>
              ...
              <label className="ml">CR Number *</label>
              <input className="mi" placeholder="e.g. CR10000001" value={form.crNo||''} onChange={e=>setForm({...form,crNo:e.target.value.toUpperCase()})}/>
              ...
              <select className="ms" value={form.type||''} onChange={e=>setForm({...form,type:e.target.value})}>
                ...
                <option>Savings Account</option><option>Checking Account</option>
                <option>Current Account</option><option>Credit Card Balance</option>
              </select>
              <div className="mf">
                <button className="mb c" onClick={closeModal}>Cancel</button>
                <button className="mb p" onClick={doNewAccount}>Create Account</button>
              </div>
            </>}
            ...
            {modal==='transaction'&&<>
              ...
              <div className="note">Enter the CR number — the system finds the account automatically</div>
              <label className="ml">CR Number *</label>
              <input className="mi" placeholder="e.g. CR10000001" value={form.crNo||''} onChange={e=>setForm({...form,crNo:e.target.value.toUpperCase()})}/>
              <label className="ml">Type *</label>
              <select className="ms" value={form.type||''} onChange={e=>setForm({...form,type:e.target.value})}>
                <option value="">Select type</option><option>Deposit</option><option>Withdrawal</option>
              </select>
              <label className="ml">Amount (LKR) *</label>
              <input className="mi" placeholder="1000" type="text" inputMode="decimal" value={form.amount||''} onChange={e=>setForm({...form,amount:e.target.value})}/>
              <label className="ml">Description</label>
              <input className="mi" placeholder="Optional" value={form.desc||''} onChange={e=>setForm({...form,desc:e.target.value})}/>
              <div className="mf">
                <button className="mb c" onClick={closeModal}>Cancel</button>
                <button className="mb p" onClick={doNewTx}>Save</button>
              </div>
            </>}
            ...
            {modal==='loan'&&<>
              ...
              <div style={{fontSize:12,color:'#64748b',marginBottom:13,textAlign:'center'}}>
                {loanStep===1?'Step 1: Select Loan Type':'Step 2: Enter Loan Details'}
              </div>
              ...
              {!newLoanRes?<>
                {loanStep===1&&<>
                  <div className="lt-grid">
                    {LOAN_TYPES.map(lt=>(
                      <div key={lt.type}
                        className="lt-card"
                        style={selType?.type===lt.type?{borderColor:lt.color,background:lt.bg}:{}}
                        onClick={()=>setSelType(lt)}>
                  ...
                  <div className="mf">
                    <button className="mb c" onClick={closeModal}>Cancel</button>
                    <button className="mb p" onClick={()=>{if(!selType){toast$('❌ Select a loan type','err');return;}setLoanStep(2);}}>Next →</button>
                  </div>
                </>}
                ...
                {loanStep===2&&<>
                  <div style={{background:selType.bg,borderRadius:10,padding:'10px 13px',marginBottom:13,display:'flex',alignItems:'center',gap:10,border:`1.5px solid ${selType.color}`}}>
                    ...
                    <div>
                      <div style={{fontWeight:700,color:selType.color}}>{selType.type}</div>
                      <div style={{fontSize:12,color:'#64748b'}}>{selType.rate}% p.a. | {selType.minM}–{selType.maxM} months</div>
                    </div>
                  ...
                  <label className="ml">CR Number *</label>
                  <input className="mi" placeholder="e.g. CR10000001" value={form.crNo||''} onChange={e=>setForm({...form,crNo:e.target.value.toUpperCase()})}/>
                  <label className="ml">Loan Amount (LKR) *</label>
                  <input className="mi" placeholder={`e.g. ${selType.min.toLocaleString()}`} type="text" inputMode="decimal"
                  ...
                  <select className="ms" value={form.months||String(selType.minM)} onChange={e=>setForm({...form,months:e.target.value})}>
                    {Array.from({length:Math.floor((selType.maxM-selType.minM)/6)+1},(_,i)=>selType.minM+i*6)
                      .map(m=><option key={m} value={m}>{m} months ({Math.floor(m/12)}yr {m%12>0?m%12+'mo':''})</option>)}
                  </select>
                  {emiPreview&&(
                  ...
                  <div className="mf">
                    <button className="mb c" onClick={()=>setLoanStep(1)}>← Back</button>
                    <button className="mb p" onClick={doNewLoan}>Apply Now</button>
                  </div>
                </>}
              </>:<>
                <div className="suc">
                  ...
                  <div className="suc-s" style={{marginTop:5,color:'#b45309',fontWeight:600}}>
                    {isAdmin?'Approve the loan to activate it':'Awaiting Admin approval'}
                  </div>
                </div>
                <div className="mf"><button className="mb p" onClick={closeModal}>Done</button></div>
              </>}
            ...
            {modal==='payemi'&&<>
              ...
              <label className="ml">Loan ID *</label>
              <input className="mi" placeholder="e.g. LN10000001" type="text" value={form.emiLoanNo||''} onChange={e=>setForm({...form,emiLoanNo:e.target.value.toUpperCase()})}/>
              <label className="ml">CR Number *</label>
              <input className="mi" placeholder="e.g. CR10000001" type="text" value={form.emiCrNo||''} onChange={e=>setForm({...form,emiCrNo:e.target.value.toUpperCase()})}/>
              <label className="ml">Payment Amount (LKR) *</label>
              <input className="mi" placeholder="e.g. 5000" type="text" inputMode="decimal" value={form.emiAmount||''} onChange={e=>setForm({...form,emiAmount:e.target.value})}/>
              <label className="ml">Note (optional)</label>
              <input className="mi" placeholder="e.g. November EMI" value={form.emiNote||''} onChange={e=>setForm({...form,emiNote:e.target.value})}/>
              {!payResult
                ...
                : <>
                  ...
                  <div className="mf">
                    <button className="mb c" onClick={()=>{setPayResult(null);setForm({});}}>Pay Another</button>
                    <button className="mb p" onClick={closeModal}>Done</button>
                  </div>
                </>
              }
            </>}
